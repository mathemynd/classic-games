...
describe('Ultimate Tic Tac Toe', () => {
  ...
  it('shows winner banner when a player wins three mini-boards in a row', () => {
    // X wins boards 0, 1, 2 (meta top row), each via middle column [1,4,7].

    // X wins board 0
    clickCell(0, 4); clickCell(4, 0);
    clickCell(0, 7); clickCell(7, 0);
    clickCell(0, 1); // X wins board 0 via [1,4,7]

    // X wins board 1
    clickCell(1, 0); clickCell(1, 4);
    clickCell(4, 1); clickCell(1, 7);
    clickCell(7, 1); clickCell(1, 1); // X wins board 1 via [1,4,7]

    // X wins board 2 (overall game-win triggers here via meta [0,1,2])
    clickCell(3, 2); clickCell(2, 1);
    clickCell(5, 2); clickCell(2, 4);
    clickCell(4, 2); // O incidentally wins board 4 here
    clickCell(2, 7); // X wins board 2 via [1,4,7] AND wins overall game

    expect(msg().classList.contains('hide')).toBe(false);
    expect(msg().classList.contains('winner-msg')).toBe(true);
    expect(msg().innerText).toMatch(/X.*win/i);
  });

  it('mini-board fills without a winner is marked as drawn and disabled', () => {
    // Target board 0: X={0,2,3,7,8}, O={1,4,5,6} — no winning pattern.

    // X fills board 0 cells 8,7,3,2,0 via round trips through other boards' cell 0
    clickCell(0, 8); clickCell(8, 0);
    clickCell(0, 7); clickCell(7, 0);
    clickCell(0, 3); clickCell(3, 0);
    clickCell(0, 2); clickCell(2, 0);
    clickCell(0, 0);

    // O fills board 0 cells 1,4,5,6
    clickCell(0, 1); clickCell(1, 0);
    clickCell(0, 4); clickCell(4, 0);
    clickCell(0, 5); clickCell(5, 0);
    clickCell(0, 6); // board 0 full → DRAW

    expect(miniBoard(0).classList.contains('won')).toBe(true);
    expect(miniBoard(0).classList.contains('draw')).toBe(true);
    expect(miniBoard(0).dataset.winner).toBeUndefined();
    for (let c = 0; c < 9; c++) {
      expect(cell(0, c).disabled).toBe(true);
    }
  });

  it('shows draw banner when all boards resolved without a meta row-winner', () => {
    // 49-move hardcoded sequence. Distribution at end of game:
    //   X wins boards {0, 1, 5, 6, 8}
    //   O wins boards {2, 3, 4, 7}
    // No 3-in-a-row in the meta-grid for either player → DRAW.

    // X wins board 0 via [1,4,7]
    clickCell(0, 1); clickCell(1, 0);
    clickCell(0, 4); clickCell(4, 0);
    clickCell(0, 7);

    // X wins board 1 via [1,4,7]
    clickCell(7, 1); clickCell(1, 4);
    clickCell(4, 1); clickCell(1, 7);
    clickCell(7, 4); clickCell(4, 3);
    clickCell(3, 1); clickCell(1, 1);

    // O wins board 2 via [0,3,6]
    clickCell(8, 2); clickCell(2, 2);
    clickCell(2, 0); clickCell(5, 2);
    clickCell(2, 1); clickCell(6, 2);
    clickCell(2, 3); clickCell(3, 2);
    clickCell(2, 6);

    // O wins board 3 via [1,4,7]
    clickCell(6, 3); clickCell(3, 4);
    clickCell(4, 5); clickCell(5, 3);
    clickCell(3, 3); clickCell(3, 7);

    // O wins board 4 via [0,4,8]
    clickCell(7, 8); clickCell(8, 4);
    clickCell(4, 2); clickCell(4, 4);
    clickCell(4, 6); clickCell(6, 5);
    clickCell(5, 4); clickCell(4, 8);

    // X wins board 5 via [2,4,6]
    clickCell(8, 5); clickCell(5, 5);
    clickCell(5, 6);

    // X wins board 6 via [0,1,2]
    clickCell(6, 6); clickCell(6, 0);
    clickCell(7, 6); clickCell(6, 1);

    // O wins board 7 via [1,4,7]
    clickCell(7, 7);

    // X wins board 8 via [6,7,8] → triggers overall DRAW
    clickCell(8, 6); clickCell(8, 0);
    clickCell(8, 7); clickCell(8, 3);
    clickCell(8, 8);

    expect(msg().classList.contains('hide')).toBe(false);
    expect(msg().classList.contains('draw-msg')).toBe(true);
    expect(msg().innerText).toMatch(/draw/i);
  });
});
